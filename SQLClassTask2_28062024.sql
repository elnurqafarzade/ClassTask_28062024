...
select e.EmployeeID, concat(e.FirstName, ' ', e.LastName) as FullName, count(o.OrderID) as OrderCount
from Employees e
Left join Orders o on e.EmployeeID = o.EmployeeID
group by e.EmployeeID, e.FirstName, e.LastName;

create view OrderTotalPriceByProduct as
select od.OrderID, p.ProductName, sum(od.Quantity * Price) as TotalPrice
...
select e.EmployeeID, concat(e.FirstName, ' ', e.LastName) as FullName, sum(od.Quantity * p.Price) as TotalSalePrice
from Employees e
...
Create view VerilenSifarislerinDeyeri as
select c.customerID, c.CustomerName, sum(od.Quantity * p.Price) as TotalOrderPrice from Customers c
...
select * from EmployeeTotalSales
select * from OrderTotalPriceByProduct
select * from EmployeeOrderCounts
select * from Top5Product
select * from VerilenSifarislerinDeyeri
select * from EnAzBirSifaris
--```
...
--** 5. ən cox satilan 5 productu tap**
